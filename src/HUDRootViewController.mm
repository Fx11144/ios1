...
- (void)viewDidLoad {
    ...
    // 注意: UIFieldEditor 是懒加载的, 此时 subviews.firstObject 可能为 nil
    ...
    // ⚠️ 关键: CAMetalLayer 优先加到 fieldEditor.layer 上
    // 如果 fieldEditor 尚未创建 (nil), 回退到 self.view.layer
    // 反编译原文: [fieldEditor.layer addSublayer:cametalLayer]
    if (fieldEditor) {
        [fieldEditor.layer addSublayer:gMetalLayer];
    } else {
        [self.view.layer addSublayer:gMetalLayer];
    }
    ...
    // ====== 步骤7: 初始化 ImGui Metal 后端 ======
    ImGui_ImplMetal_Init(device);
    
    // ====== 步骤8: 启动 DisplayLink 60fps 帧循环 ======
...
- (void)ChangeUI {
    ...
    // ImGui Metal 后端帧开始 (需要 renderPassDescriptor, 不是 drawable)
    ImGui_ImplMetal_NewFrame(renderPassDesc);
    
    // 1. ImGui 帧开始 (CAMetalDrawable 无 .size, 从 texture 取)
